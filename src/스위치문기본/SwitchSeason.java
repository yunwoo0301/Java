...
// 해당하는 케이스가 없는 경우를 위해 default 문이 필요 ( if ~ else if ~ else 동일한 용도)
public class SwitchSeason {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.print("계절을 입력 하세요 : ");
        String season = sc.next(); // 공백 기준으로 문자열을 입력
        switch (season) {
            ...
            case "winter" : case "WINTER" :
                System.out.println("아직 추운 겨울입니다.");
                break;
            default:
                System.out.println("계절을 잘못 입력하였습니다.");
